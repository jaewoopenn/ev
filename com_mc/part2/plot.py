...
def main():
    ...
    if os.path.exists(csv_ph_path):
        ...
        for m, data in data_by_m_ph.items():
            plt.figure(figsize=(10, 6))
            plt.plot(data["p_h_values"], data["rates_orig"], marker='o', linestyle='-', color='gray', label='1. FFD (Original Math)')
            plt.plot(data["p_h_values"], data["rates_ffd_new"], marker='x', linestyle=':', color='blue', label='2. FFD (New Math)')
            plt.plot(data["p_h_values"], data["rates_mb_new"], marker='s', linestyle='--', color='red', linewidth=2, label='3. MB-FFD (New Math)')
            plt.plot(data["p_h_values"], data["rates_cu_udp_orig"], marker='^', linestyle='-', color='green', linewidth=2, label='4. CU-UDP (Original Math)')
            plt.title(f'Multiprocessor (m={m}, Util=0.80) Acceptance Ratio vs P_H', fontsize=14)
            plt.xlabel('Probability of HC Task (P_H)', fontsize=12)
            plt.ylabel('Acceptance Ratio (%)', fontsize=12)
            plt.legend(fontsize=12)
            plt.grid(True, linestyle=':', alpha=0.7)
