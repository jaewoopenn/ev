...
class IMCTaskSetGenerator:
    ...
    def generate_single_taskset(
        ...
        """
        Generate a single IMC task set.

        Args:
            taskset_id: unique identifier
            util_cap:   target utilization cap (used as U_target for UUniFast)
        ...
        # Step 2: UUniFast to distribute util_cap across n tasks
        base_utils = uunifast_discard(n, util_cap, self.rng)

        # Step 3: Assign criticality and compute execution times
        tasks = []
        for i in range(n):
            u_i = base_utils[i]

            # Period: uniform integer in [period_min, period_max]
            ...
            if is_HC:
                # HC task: C_LO = u_i * T_i, C_HI = R * C_LO
                R = self.rng.uniform(self.R_range[0], self.R_range[1])
                C_LO = max(1, round(u_i * T_i))
                C_HI = max(C_LO, round(R * C_LO))  # ensure C_HI >= C_LO
                ...
                # LC task: C_A = u_i * T_i, C_D = lambda * C_A
                lam = self.rng.uniform(
                    self.lambda_range[0], self.lambda_range[1]
                )
                C_LO = max(1, round(u_i * T_i))  # C_A (active)
                C_HI = max(0, round(lam * C_LO))  # C_D (degraded), can be 0
        ...
        # Step 4: Create TaskSet and compute utilizations
        ts = TaskSet(
