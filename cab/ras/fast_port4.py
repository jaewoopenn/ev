...
import ras.fast_port_def4 as port
...
def run_simulation(ev_set: List[EVRequest], algorithm: str) -> bool:
    ...
    while finished_cnt < total_evs:
        ...
        if algorithm == 'Fluid-ZL': # 이름 변경
            allocated_powers = port.calculate_fluid_edf_zl_power(
...
if __name__ == '__main__':
    ...
    # 결과 저장소
    results = {l: {k: 0 for k in ['EDF', 'LLF', 'Fluid-ZL']} for l in range(STRESS_START, STRESS_START + STRESS_NUM)}
    ...
    ratios = {k: [] for k in ['EDF', 'LLF', 'Fluid-ZL']}
    ...
    if levels:
        ...
        plt.plot(levels, ratios['Fluid-ZL'], marker='^', label=NAME_NEW_ALGO, linestyle='-', color='green', linewidth=2)
